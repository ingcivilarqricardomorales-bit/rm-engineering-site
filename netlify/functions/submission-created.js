...
// Utilidad simple por si Netlify cambia el shape
const pick = (obj, keys = []) => {
  for (const k of keys) {
...
export const handler = async (event) => {
  try {
    ...
    // Mezclamos posibles capas (por robustez)
    const fields = { ...data, ...payload, ...parsed };

    const name    = pick(fields, ["name", "Nombre", "fullName", "fullname"]) || "-";
    const email   = pick(fields, ["email", "Email", "correo", "mail"])        || "-";
    const subject = pick(fields, ["subject", "Subject", "asunto", "Asunto"])  || "Solicitud de cotización";
    const message = pick(fields, ["message", "Mensaje", "msg", "Message", "comments"]) || "-";

    sgMail.setApiKey(process.env.SENDGRID_API_KEY);

    const FROM_EMAIL = process.env.FROM_EMAIL; // Single Sender VERIFICADO
    const TO_EMAIL   = process.env.TO_EMAIL || "ing.civil.arq.ricardo.morales@gmail.com";
